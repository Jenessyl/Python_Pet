...
void game_loop(std::string petName, char animalType) {
  bool outerFlag = true;
  char animal = animalType;
  int day = 1;
  int moves;
  ...
  while (outerFlag == true) {
      ...
      do {
        ...
        if ((actionChoice != 'a') && (actionChoice != 'b') &&
          ...
          case 'a':
            ...
            color_text("❥ " + petName + " thanks you for playing with it ❥",
                       'C');
          ...
          case 'b':
            feed(petName, petBars, animal);
            actionFlag = false;
            moves -= 1;
            break;

          case 'c':
            clean_action(petName, petBars, animal);
            actionFlag = false;
            moves -= 1;
            break;

          case 'd':
            sleep(petName, petBars, animal);
            actionFlag = false;
            moves -= 1;
            break;
    ...
    if (petBars.getHealth() <= 0) {
      displayDeath(petName, std::to_string(day), animal);
      deathFlag = true;
      outerFlag = false;
    }
    ...
    if ((petBars.getHappiness() >= 100) && (deathFlag == false)) {
      displayWin(petName, std::to_string(day), animal);
      outerFlag = false;
    }
